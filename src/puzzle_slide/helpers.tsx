...
export function getIndex(row:any, col:any) {
  return parseInt(row, 10) * GRID_SIZE + parseInt(col, 10);
}

// Get the row/col pair from a linear index.
export function getMatrixPosition(index:any) {
  return {
...
export function getVisualPosition(row:any, col:any, width:any, height:any) {
  return {
    x: col * width,
    y: row * height,
  };
...
export function shuffle(tiles:any):any {
  const shuffledTiles = [
    ...tiles
      .filter((t:any) => t !== tiles.length - 1)
...
export function canSwap(srcIndex:any, destIndex:any) {
...
export function swap(tiles:any, src:any, dest:any) {
  const tilesResult = [...tiles];
  [tilesResult[src], tilesResult[dest]] = [tilesResult[dest], tilesResult[src]];
  return tilesResult;
}

export function updateURLParameter(url:any, param:any, paramVal:any) {

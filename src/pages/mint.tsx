...
import { Cutomizing_piece_contractAddress, Cutomizing_piece_abi } from "data/contract"
...
const Mint: NextPage = () => {
  ...
  async function publicMint(){
    ...
    let account = user.get('ethAddress');
	  let contract = new window.web3.eth.Contract(Cutomizing_piece_abi,Cutomizing_piece_contractAddress);
  ...
  if(!isAuthenticated){
    return(
     <>
            ...
            <p>
            <button onClick={() => authenticate({signingMessage: "Sign to login to Dashbard3" })}>
  ...
  return (
    <Layout title="Mint" hasHeader>
      ...
      <h2>
        ...
        <p>
        <input type="number" step={1} onChange={handleChange} id="amount" name="amount" placeholder="1" />
        </p>
        <p>
